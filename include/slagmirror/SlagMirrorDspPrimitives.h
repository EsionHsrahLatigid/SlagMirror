...
/** A stereo sample used by the SlagMirror DSP engine. */
struct StereoFrame
{
    float left = 0.0f;
    float right = 0.0f;
...
/** One-pole low-pass used as a cheap spectral-tilt proxy, not an FFT stage. */
...
public:
    void prepare (double sampleRate, float cutoffHz) noexcept
    {
        const auto safeRate = std::isfinite (sampleRate) && sampleRate > 1.0 ? sampleRate : 44100.0;
        const auto nyquist = static_cast<float> (safeRate * 0.5);
        const auto safeCutoff = clampFinite (cutoffHz, 10.0f, nyquist * 0.45f, 1200.0f);
        coefficient = std::exp (-2.0f * std::numbers::pi_v<float> * safeCutoff / static_cast<float> (safeRate));
    }
    ...
    {
        const auto safeInput = std::isfinite (input) ? input : 0.0f;
        state = (1.0f - coefficient) * safeInput + coefficient * state;
        if (std::fabs (state) < 1.0e-20f)
            state = 0.0f;
...
/** First-order all-pass scatter cell for reflective stereo smearing. */
class AllPassScatter
{
public:
    void reset() noexcept
    {
    ...
    {
        const auto safeInput = std::isfinite (input) ? input : 0.0f;
        const auto safeCoefficient = clampFinite (coefficient, -0.86f, 0.86f, 0.0f);
        const auto output = delay - safeCoefficient * safeInput;
        delay = safeInput + safeCoefficient * output;
        if (std::fabs (delay) < 1.0e-20f)
            delay = 0.0f;
        return std::isfinite (output) ? output : 0.0f;
    }
